...
const CyclingDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const cyclingActivities = dataList.filter((activity) => activity.activityType === 'cycling'); // filtering activity type
    setFilteredData(cyclingActivities);
  }, [dataList]);
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      try {
        await axios.post(API + '/api/tasks/cycling', formData, {
          headers: { Authorization: `Bearer ${validate}` },
        });
        fetchData();
        setModalOpen(false);
